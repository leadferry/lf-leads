...
class LF_Mailpoet extends LF_Lead_Capture {
	...
	public function lf_lead_form_id_callback() {
		...
		echo '<input id="lf_lead_form_id" name="lf_mailpoet_options[lead_form_id]" size="40" type="text" value="'. $lead_form_id .'">';
	}
	...
	public function lf_lead_first_name_callback() {
		$options = get_option( 'lf_mailpoet_options' );
		$lead_first_name = isset( $options['lead_first_name'] ) ? $options['lead_first_name'] : 'wysija[user][firstname]' ;
		echo '<input id="lf_lead_first_name" name="lf_mailpoet_options[lead_first_name]" size="40" type="text" value="'. $lead_first_name .'">';
	}
	...
	public function lf_lead_last_name_callback() {
		$options = get_option( 'lf_mailpoet_options' );
		$lead_last_name = isset( $options['lead_last_name'] ) ? $options['lead_last_name'] : 'wysija[user][lastname]' ;
		echo '<input id="lf_lead_last_name" name="lf_mailpoet_options[lead_last_name]" size="40" type="text" value="'. $lead_last_name .'">';
	}
	...
	public function lf_lead_email_callback() {
		...
		echo '<input id="lf_lead_email" name="lf_mailpoet_options[lead_email]" size="40" type="text" value="'. $lead_email .'">';
	}
	...
	public function add_scripts() {
		...
		wp_localize_script( 'lf_mailpoet', 'local_data', $local_data );
	}
}
