...
class Test_LF_Pardot extends WP_UnitTestCase{

    function setUp() {
		parent::setUp();
		$product = array( 'selected_product' => 'pardot' );
		lf_support( $product );
        $this->selected_product = new LF_Pardot();
	}
    function test_pardot_capture_lead() {
        ...
        $mock = $this->getMockBuilder('LF_Pardot')
    ...
    function test_settings_section_text() {
        $output = '<h2>Pardot Settings</h2><p>Please provide the values of name attributes for the follwing fields in your Pardot form. </p>';
        $this->expectOutputString( $output, $this->selected_product->settings_section_text() );
    }

    function test_lf_lead_form_id_callback() {
        update_option('lf_pardot_options', array('lead_form_id' => '1') );
        $output = '<input id="lf_lead_form_id" name="lf_pardot_options[lead_form_id]" size="40" type="text" value="1">';
        $this->expectOutputString( $output, $this->selected_product->lf_lead_form_id_callback() );
    }

    function test_lf_lead_first_name_callback() {
        update_option('lf_pardot_options', array('lead_first_name' => 'firstname') );
        $output = '<input id="lf_lead_first_name" name="lf_pardot_options[lead_first_name]" size="40" type="text" value="firstname">';
        $this->expectOutputString( $output, $this->selected_product->lf_lead_first_name_callback() );
    }

    function test_lf_lead_last_name_callback(){
        update_option('lf_pardot_options', array('lead_last_name' => 'lastname') );
        $output = '<input id="lf_lead_last_name" name="lf_pardot_options[lead_last_name]" size="40" type="text" value="lastname">';
        $this->expectOutputString( $output, $this->selected_product->lf_lead_last_name_callback() );
    }

    function test_lf_lead_email_callback(){
        update_option('lf_pardot_options', array('lead_email' => 'email') );
        $output = '<input id="lf_lead_email" name="lf_pardot_options[lead_email]" size="40" type="text" value="email">';
    ...
    function test_add_scripts() {
        ...
        $this->assertTrue( wp_script_is( 'lf_pardot', 'enqueued') );
    }
}
